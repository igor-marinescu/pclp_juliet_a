...
# Ignored Modules
#-------------------------------------------------------------------------------
# Argument 2 (ignored modules) passed? 
if [[ "$#" -gt 1 ]]; then
    IGNORED_MODULES=$(realpath "$2")
    if [[ ! -f "$IGNORED_MODULES" ]]; then
        echo "[ERROR] Ignored modules file doesn't exist: $IGNORED_MODULES"
        exit 1
    fi
fi

if [[ ! -z "$IGNORED_MODULES" ]]; then
    echo "[INFO] Ignored modules file: $IGNORED_MODULES"
fi
...
#-------------------------------------------------------------------------------
# Find all Makefiles in all subdirectories and store them in a temporary file
# "makefiles.txt_0". After processing every line of the file, a second file
# is created "makefiles.txt" - where the ignored modules are removed.
#-------------------------------------------------------------------------------
find "$WORKING_DIR" -name "Makefile" > "$MAKEFILES_FILE"_0

MAKEFILES_COUNT=$(wc -l < "$MAKEFILES_FILE"_0)
if [[ $MAKEFILES_COUNT -le 0 ]]; then
    echo "[INFO] No Makefiles found, nothing to do"
    exit 0
fi

# Create second (final) file of makefiles
touch "$MAKEFILES_FILE"
...
while read make_file; do
    ...
    # Check if Makefile not in list of ignored files
    if [[ ! -z "$IGNORED_MODULES" ]]; then
        cd "$WORKING_DIR"
        python3 "$SCRIPT_PATH/scripts/ignore_list.py" "$WORKING_DIR" "$IGNORED_MODULES" "$make_file"
        if [[ $? -ne 0 ]]; then
            echo "[Info] Module ignored (its name is in ignored list)"
            continue
        fi
    fi

    # If the Makefile not ignored, insert its name in the second list of makefiles
    echo $make_file >> "$MAKEFILES_FILE"
...
done < "$WORKING_DIR/$MAKEFILES_FILE"_0

# Interpret the output generated by PC-lint
cd "$WORKING_DIR"
python3 "$SCRIPT_PATH/scripts/pclp_a_main.py" . "$IGNORED_MODULES"
